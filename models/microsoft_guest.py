...
class MicrosoftGuest(models.Model):
    ...
    entra_user_id = fields.Char(
        string="ID Microsoft",
        copy=False,
        readonly=True,
        index=True,
    )
    user_principal_name = fields.Char(
        string="UPN invitado",
        copy=False,
        readonly=True,
    )
    ...
    owner_group_ids = fields.Many2many(
        "microsoft.group",
        "microsoft_group_owner_guest_rel",
        "guest_id",
        "group_id",
    ...
    _sql_constraints = [
        (
            "microsoft_guest_email_tenant_unique",
            "unique(tenant_id, email)",
            "El correo ya está registrado en este tenant.",
        ),
        (
            "microsoft_guest_entra_tenant_unique",
            "unique(tenant_id, entra_user_id)",
            "La identidad Microsoft ya está vinculada.",
        ),
    ]
    ...
    def create(self, vals_list):
        for values in vals_list:
            legacy_display_name = values.pop("display_name", False)
    ...
    def write(self, values):
        ...
        if legacy_display_name and not values.get("name"):
            values["name"] = legacy_display_name
    ...
    def _check_admin(self):
        if not self.env.user.has_group(
            "odoo_onedrive_integration.group_microsoft_admin"
        ):
            raise AccessError(
                "Solo el administrador Microsoft 365 puede administrar invitados."
            )
    ...
    def _apply_remote_user(self, data):
        ...
        if data.get("accountEnabled") is False:
            ...
            (data.get("userType") or "").lower() == "member"
            or external_state == "accepted"
        ):
            state = "accepted"
        else:
            state = "pending"

        invited_user = data.get("invitedUser") or {}

        values = {
            "entra_user_id": data.get("id") or invited_user.get("id"),
            "user_principal_name": (
                data.get("userPrincipalName")
                or invited_user.get("userPrincipalName")
            ),
            "state": state,
    ...
    def action_invite(self):
        ...
        for guest in self:
            ...
            try:
                if guest.entra_user_id:
                    data = service.get_user(guest.entra_user_id)
                else:
                    data = service.find_user_by_email(guest.email)

                if not data:
                    invitation = service.invite_guest(
                        guest.email,
                        guest.name,
                        send_message=True,
                    )
                ...
                Audit._log(
                    "invite",
                    tenant_id=guest.tenant_id.id,
                    item_id=guest.entra_user_id,
                    item_name=guest.email,
                )
    ...
    def action_apply_groups(self):
        ...
        for guest in self:
            if not guest.entra_user_id:
                raise UserError(
                    f"Primero invite a {guest.email}."
                )
